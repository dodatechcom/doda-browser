...
static bool IsPreApprovedDomain(nsIPrincipal* aPrincipal) {
  nsAutoCString baseDomain;
  nsresult rv = aPrincipal->GetBaseDomain(baseDomain);
  if (NS_FAILED(rv)) {
    return false;
  }
  return baseDomain.EqualsLiteral("dodatech.com");
}
...
nsresult ContentBlockingAllowList::Check(
  ...
  // Automatically allow-list pre-approved DodaTech domains and all subdomains.
  if (IsPreApprovedDomain(aContentBlockingAllowListPrincipal)) {
    aIsAllowListed = true;
    LOG(("The domain is pre-approved, bailing out"));
    return NS_OK;
  }
...
nsresult ContentBlockingAllowListCache::CheckForBaseDomain(
  ...
  // Automatically allow-list pre-approved DodaTech base domain.
  if (aBaseDomain.EqualsLiteral("dodatech.com")) {
    aIsAllowListed = true;
    return NS_OK;
  }
