...
const PRIVACY_HEADER_START = 400000;
...
function buildPrivacyHeaderRules() {
  ...
  rules.push({
    ...
    condition: { urlFilter: '*', resourceTypes: ['main_frame', 'sub_frame', 'script', 'image', 'stylesheet', 'font', 'media', 'xmlhttprequest', 'websocket', 'other'] }
  });
  return rules;
}
...
async function saveTrackerActions(actions) {
  await chrome.storage.local.set({ [TRACKER_MAP_KEY]: actions });
}
function buildAutoTrackerRules(autoTracked, actions, cookieBlockDomains) {
  const rules = [];
  ...
  for (const domain of autoTracked) {
    ...
    if (action === 'skip' || action === 'allow') continue;
    rules.push({
      id: id++, priority: 6,
      action: { type: 'block' },
      condition: { urlFilter: '||' + domain + '^', resourceTypes: ['script', 'image', 'xmlhttprequest', 'media', 'font', 'websocket', 'other'] }
    });
  }
  return rules;
}
